...
public class TypeTemplateServiceImpl extends ServiceImpl<TypeTemplateMapper, TypeTemplate> implements TypeTemplateService {
    ...
    public List<Map> findSpecList(Long typeId) {
        ...
        specs.forEach(s -> {
            ...
            queryWrapper.eq("spec_id", Long.parseLong(String.valueOf(s.get("id"))));
            List<SpecificationOption> optionList = specificationOptionMapper.selectList(queryWrapper);
            s.put("options",optionList);
        });
        return specs;
    }
}
